...
function(lang, declare, query) {
	...
	declare("graph.SourcePage", null, {
		...
		display : function(holder, callback) {
			...
			holder.innerHTML = "<div class='frame source'><h1>" + this.title + "</h1><script type='syntaxhighlighter' class='brush: java'><![CDATA[" + code
					+ "]]></script></div>";
		...
		next : function() {
			if (this.currentLineNumber < this.lineCount) {
				...
				var codes = query("code", nextLine[1]);
				while (codes.length == 0 || (query("code.comments", nextLine[1]).length > 0) && codes.length == 1) {
					if (this.currentLineNumber == this.lineCount) {
						return false;
					}
					this.currentLineNumber++;
					nextLine = query(".number" + this.currentLineNumber, this.holder);
					codes = query("code", nextLine[1]);
				}
